...
internal class ScannerCommandHandler(
    ...
    fun cancelScan(result: Result) {
        scannerSessionProvider()?.scannerView?.pauseScan()
        result.success(true)
    }
    ...
    fun setScanDelay(call: MethodCall, result: Result) {
        ...
        scannerSessionProvider()?.scannerView?.updateScanPeriod(delay.toInt())
        result.success(true)
    }
    ...
    fun setCropArea(call: MethodCall, result: Result) {
        ...
        scannerSessionProvider()?.scannerView?.setCropArea(cropRect)
        result.success(true)
    }
    ...
    private fun activeScannerView(result: Result, errorMessage: String): ScannerView? {
        val scannerView = scannerSessionProvider()?.activeScannerView
        if (scannerView == null) {
